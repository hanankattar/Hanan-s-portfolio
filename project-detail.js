...
function renderHeroImage(project){
  if (project.cover){
    return `<div class="hero-image"><img src="${escapeHtml(normalizeImageUrl(project.cover))}" alt="${escapeHtml(project.title)} cover photo"></div>`;
  }
...
function renderGallery(project){
  ...
  const items = project.gallery.map(src =>
    `<div class="g-item"><img src="${escapeHtml(normalizeImageUrl(src))}" alt="${escapeHtml(project.title)} screenshot"></div>`
  ).join('');
  return `
    <h3>Screens</h3>
