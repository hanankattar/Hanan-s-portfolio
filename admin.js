...
const els = {
  ...
  galleryLinks: document.getElementById('f-gallery-links'),
...
function resetForm(){
  ...
  els.coverUploadStatus.textContent = '';
  els.galleryUploadStatus.textContent = '';
...
function startEdit(id){
  ...
  const galleryLinks = (project.gallery || []).filter(g => !g.startsWith('data:'));
  ...
  els.galleryLinks.value = galleryLinks.join('\n');
  ...
  if (project.cover){
    ...
    els.coverPreviewImg.src = normalizeImageUrl(project.cover);
  } else {
    els.coverPreviewWrap.style.display = 'none';
  }
...
els.coverUpload.addEventListener('change', async () => {
  const file = els.coverUpload.files[0];
  if (!file) return;
  els.coverUploadStatus.textContent = 'Processing photo…';
  try {
    coverDataUrl = await resizeImageFile(file);
    els.coverPreviewWrap.style.display = '';
    els.coverPreviewImg.src = coverDataUrl;
    els.coverPath.value = '';
    els.coverUploadStatus.textContent = 'Ready.';
  } catch (err){
    els.coverUploadStatus.textContent = "Couldn't process that photo — try a different file, or use the link field instead.";
    coverDataUrl = "";
  }
});

els.galleryUpload.addEventListener('change', async () => {
  const files = Array.from(els.galleryUpload.files || []);
  if (files.length === 0) return;
  els.galleryUploadStatus.textContent = 'Processing photos…';
  let failed = 0;
  ...
  els.galleryUploadStatus.textContent = failed > 0
    ? `Added ${files.length - failed} photo(s) — ${failed} couldn't be processed.`
    : 'Ready.';
});

els.coverPath.addEventListener('blur', () => {
  const val = els.coverPath.value.trim();
  if (!val) return;
  coverDataUrl = "";
  els.coverPreviewWrap.style.display = '';
  els.coverPreviewImg.src = normalizeImageUrl(val);
});

els.form.addEventListener('submit', async (e) => {
  e.preventDefault();

  const cover = coverDataUrl || normalizeImageUrl(els.coverPath.value.trim());
  const linkGallery = els.galleryLinks.value
    .split('\n')
    .map(s => s.trim())
    .filter(Boolean)
  ...
  const project = {
    ...
    code: els.code.value,
    cover: cover,
    gallery: galleryDataUrls.concat(linkGallery),
    links: {
  ...
  try {
    ...
    if (err && err.message === 'STORAGE_FULL'){
      els.formStatus.textContent = "Browser storage is full — try using image links instead of uploads for this project (or for older ones), or remove a photo and try again.";
    } else {
      els.formStatus.textContent = "Something went wrong saving this project — please try again.";
    }
  }
});
